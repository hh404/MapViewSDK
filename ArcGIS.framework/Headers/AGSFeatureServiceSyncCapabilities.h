...
/** Indicates whether the service supports registering existing datasets (*.geodatabase file)
 as new replicas. This is useful when you already have an existing replica you want to use it
...
/** Indicates whether the service supports sync capabilities independently on a layer-by-layer basis in
 the replica. For example, one layer can upload changes while another downloads during sync. A perLayer sync model also allows clients to limit the sync to a subset of layers in the replica.
 @see AGSGDBSyncModel
 @since 10.2
 */
@property (nonatomic, assign) BOOL supportsPerLayerSync;

/** Indicates whether the service supports sync capabilities for the entire replica.
 All layers in the replica will be synchronized together.
 @see AGSGDBSyncModel
 @since 10.2
 */
@property (nonatomic, assign) BOOL supportsPerReplicaSync;
